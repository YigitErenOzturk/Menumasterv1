...
(function () {
  ...
  const renderRestaurants = (restaurants, isShowingMore = false) => {
    ...
    if (!allRestaurants || allRestaurants.length === 0) {
      listEl.innerHTML =
        '<p class="col-span-full text-center text-red-500 font-medium py-10">No restaurants found matching your criteria.</p>';
  ...
  const fetchRestaurants = async () => {
    ...
    if (listEl) {
      listEl.innerHTML =
        '<p class="col-span-full text-center text-gray-500 py-10 animate-pulse">Loading Restaurants...</p>';
    }
    ...
    try {
      ...
      if (listEl) {
        listEl.innerHTML =
          '<p class="col-span-full text-center text-red-500 py-10">Failed to load restaurants. Please try again later.</p>';
      }
    }
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // <script>
     // document.addEventListener('DOMContentLoaded', async () => {
       ...
       // const API_URL = 'http://localhost:5000/api/restaurants/all'; 
       ...
       // try {
         ...
         // if (!Array.isArray(restaurants) || restaurants.length === 0) {
            // listElement.innerHTML = '<p class="col-span-full text-center text-gray-500">No popular restaurants found right now.</p>';
            // return;
         // }

         // listElement.innerHTML = restaurants.map(r => `
           // <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
             //   <div class="relative h-48 overflow-hidden">
               //     <img src="${r.imageUrl || 'https://placehold.co/600x400?text=Restaurant'}" alt="${r.name}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                 //    <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-orange-600 shadow-sm">
                   //     ⭐ ${r.rating || 'New'}
                  //  </div>
              //  </div>
                // <div class="p-5">
                  //  <h3 class="text-xl font-bold text-gray-900 mb-1 group-hover:text-orange-600 transition-colors">${r.name}</h3>
                   // <p class="text-gray-500 text-sm mb-4 line-clamp-2">${r.cuisine || 'Delicious Food'}</p>
                   // <div class="flex justify-between items-center border-t border-gray-100 pt-4">
                     //   <span class="text-sm font-medium text-gray-400 flex items-center gap-1">
                       //    📍 ${r.city || 'City'}
                       // </span>
                       // <a href="../restaurantfiles/restaurant-details-withoutlogin.html?id=${r.id}" class="px-4 py-2 bg-orange-50 text-orange-600 rounded-lg text-sm font-bold hover:bg-orange-600 hover:text-white transition-all">
                         //   View Menu
                       // </a>
                   // </div>
               // </div>
       ...
       // } catch (error) {
         // console.error('Fetch Error:', error);
        //  listElement.innerHTML = '<p class="col-span-full text-center text-red-500">Failed to load restaurants. Connection error.</p>';
      //  }
     // });