...
const createRestaurantCards = (restaurants) => {
  if (!Array.isArray(restaurants) || restaurants.length === 0) {
    return '<p class="col-span-4 text-center text-gray-500">No restaurants found.</p>';
  }

  return restaurants.map(r => {
    const badgeText = r.rating ? `⭐ ${r.rating}` : 'New';
    return `
      <a href="../restaurantfiles/restaurant-details.html?id=${r.id}&name=${encodeURIComponent(r.name)}" class="block bg-white rounded-xl shadow-md overflow-hidden transform hover:scale-[1.02] transition duration-300 group border border-orange-100">
        <div class="relative h-48 overflow-hidden">
          <img src="${r.imageUrl || 'https://placehold.co/400x200?text=Restaurant'}" alt="${escapeHtml(r.name)}" class="w-full h-full object-cover group-hover:opacity-80 transition-opacity">
          <div class="absolute top-2 right-2 bg-orange-600 text-white text-xs font-bold px-2 py-1 rounded shadow-md">${badgeText}</div>
        </div>
        <div class="p-4">
          <h3 class="text-lg font-bold text-orange-600 truncate">${escapeHtml(r.name)}</h3>
          <p class="text-gray-500 text-sm mt-1 uppercase tracking-wider text-xs font-semibold">${escapeHtml(r.cuisine || 'Restaurant')}</p>
        </div>
      </a>
    `;
  }).join('');
...
const renderDashboardView = async () => {
  DOM.mainContent.innerHTML = `
    <div class="animate-fade-in">
      <h1 class="text-3xl font-extrabold text-gray-900 mb-6">Discover Restaurants</h1>
      <div class="mb-8">
        <label for="city-select" class="text-sm font-bold text-gray-700">Filter by City:</label>
        <select id="city-select" class="mt-2 w-full max-w-sm p-3 text-gray-900 bg-white border border-orange-200 rounded-lg focus:ring-orange-500 focus:border-orange-500 transition-colors outline-none font-medium">
      ...
      <div id="restaurant-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div class="col-span-4 flex justify-center py-12"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600"></div></div>
      </div>
    </div>`;
  ...
  const loadRestaurants = async (filter = '') => {
    ...
    grid.innerHTML = '<div class="col-span-4 flex justify-center py-12"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600"></div></div>';
...
const renderReservationsView = async () => {
  DOM.mainContent.innerHTML = `
    <div class="animate-fade-in">
      <h1 class="text-3xl font-extrabold text-gray-900 mb-6 uppercase tracking-tight">My Reservations</h1>
      <div id="reservations-list" class="grid grid-cols-1 gap-4">
        <div class="flex justify-center py-12"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600"></div></div>
      </div>
    </div>`;
  ...
  if (reservations.error || !Array.isArray(reservations) || reservations.length === 0) {
    listEl.innerHTML = `
      <div class="text-center py-16 bg-orange-50/50 rounded-2xl border-2 border-dashed border-orange-200">
        <svg class="w-16 h-16 text-orange-200 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        <p class="text-gray-500 text-lg font-medium">You don't have any reservations yet.</p>
        <button onclick="document.getElementById('nav-dashboard').click()" class="mt-4 text-orange-600 hover:text-orange-700 font-bold">Discover Restaurants →</button>
      </div>`;
    return;
  }
  ...
  listEl.innerHTML = reservations.map(res => {
    const statusClass = 
        ...
        res.status === 'Declined' ? 'bg-red-100 text-red-700 border-red-200' : 
        'bg-orange-100 text-orange-700 border-orange-200'; 
    
    return `
    <div class="bg-white p-6 rounded-2xl border border-orange-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-orange-400 transition-colors shadow-lg">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="text-xl font-bold text-gray-900">${escapeHtml(res.restaurantName)}</h3>
          <div class="flex items-center gap-3 mt-1">
            ...
            <span class="text-gray-500 text-sm font-medium">${res.peopleCount} People</span>
          </div>
        </div>
      </div>
      <div class="flex items-center gap-4 w-full md:w-auto border-t md:border-t-0 border-orange-50 pt-4 md:pt-0">
        <span class="px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border ${statusClass}">
          ${res.status || 'Pending'}
        </span>
        <button onclick="cancelReservation(${res.id})" class="ml-auto md:ml-0 text-gray-400 hover:text-red-500 p-2 transition-colors">
...
const renderReviewsView = async () => {
  DOM.mainContent.innerHTML = `<div class="animate-fade-in"><h1 class="text-3xl font-extrabold text-gray-900 mb-6 uppercase">My Reviews</h1><div id="reviews-list" class="space-y-4">Loading...</div></div>`;
  const reviews = await fetchData('reviews');
  const listEl = document.getElementById('reviews-list');

  if (reviews.error || !Array.isArray(reviews) || reviews.length === 0) {
    listEl.innerHTML = `<p class="text-center text-gray-500 py-8">No reviews written yet.</p>`;
    return;
  }

  listEl.innerHTML = reviews.map(rev => `
    <div class="bg-white p-6 rounded-lg border border-orange-100 shadow-md">
      <h3 class="text-lg font-bold text-orange-600">${escapeHtml(rev.restaurantName)}</h3>
      <p class="text-gray-700 mt-2 font-medium italic">"${escapeHtml(rev.comment)}"</p>
    </div>
  `).join('');
};
...
const renderSettingsView = async () => {
  ...
  DOM.mainContent.innerHTML = `
    <div class="animate-fade-in max-w-4xl mx-auto p-4">
      <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 border-b border-orange-100 pb-6">
        <div>
          <h1 class="text-3xl font-extrabold text-gray-900 uppercase tracking-tight">Account Settings</h1>
          <div class="flex items-center gap-2 mt-2 text-orange-600">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            <span class="text-xs font-bold uppercase tracking-widest">Member Since: ${registrationDate}</span>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form id="settings-form" class="space-y-5">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <input type="text" id="set-name" value="${escapeHtml(userData.name)}" class="w-full p-3 bg-gray-50 border border-orange-100 rounded-xl text-gray-900 font-medium outline-none focus:border-orange-500">
                </div>
                <div>
                  <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Username</label>
                  <input type="text" id="set-username" value="${escapeHtml(userData.username)}" class="w-full p-3 bg-gray-50 border border-orange-100 rounded-xl text-gray-900 font-medium outline-none focus:border-orange-500">
                </div>
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Email Address</label>
                <input type="email" id="set-email" value="${escapeHtml(userData.email)}" class="w-full p-3 bg-gray-50 border border-orange-100 rounded-xl text-gray-900 font-medium outline-none focus:border-orange-500">
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Phone Number</label>
                <input type="text" id="set-phone" value="${escapeHtml(userData.phoneNumber || '')}" class="w-full p-3 bg-gray-50 border border-orange-100 rounded-xl text-gray-900 font-medium outline-none focus:border-orange-500">
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Address</label>
                <textarea id="set-address" rows="3" class="w-full p-3 bg-gray-50 border border-orange-100 rounded-xl text-gray-900 font-medium outline-none resize-none focus:border-orange-500">${escapeHtml(userData.address || '')}</textarea>
              </div>
              <button type="submit" class="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-4 rounded-xl transition-all shadow-md">SAVE CHANGES</button>
            </form>
        ...
        <div class="space-y-6">
          <div class="bg-white p-8 rounded-2xl border border-orange-100 shadow-xl text-center">
            <h2 class="text-lg font-bold text-gray-900 mb-4 italic">Security</h2>
            <button id="forgot-pass-btn" class="w-full border-2 border-red-100 text-red-500 hover:bg-red-500 hover:text-white font-bold py-3 rounded-xl transition-all">Send Reset Link</button>
            <div id="forgot-message" class="mt-4 text-center text-xs font-bold uppercase tracking-widest"></div>
          </div>
        </div>
      </div>
    </div>`;
...
const attachSettingsListeners = (userId, userEmail) => {
  ...
  if (settingsForm) {
    settingsForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        msg.className = "mt-4 text-center text-gray-500 font-bold";
  ...
  if (forgotBtn) {
    forgotBtn.onclick = async () => {
      ...
      fMsg.className = "mt-4 text-center text-gray-500 animate-pulse font-bold";
      ...
      try {
        ...
        if (res.ok) {
          ...
          fMsg.className = "mt-4 text-center text-red-500 font-bold";
        }
      } catch (err) {
...
const handleNavigation = (e) => {
  ...
  Object.values(DOM.navLinks).forEach(l => {
    if (l) l.classList.remove('bg-orange-600', 'text-white', 'font-bold');
  });
  e.currentTarget.classList.add('bg-orange-600', 'text-white', 'font-bold');
