...
const checkAuth = () => {
    ...
    if (!token) {
        ...
        // For a real app, you would redirect to login
        // window.location.href = 'login.html';
    }
};

const setUserInfo = () => {
    const userName = localStorage.getItem('userName'); // Fetch username from storage
    if (userName) {
        userNameEl.textContent = userName;
        userInitialEl.textContent = userName.charAt(0).toUpperCase();
    } else {
        // If no user is logged in or name is not set, show a generic state
...
const fetchData = async (endpoint) => {
    // Static data for preview purposes for cities and restaurants
    if (endpoint === 'cities') {
    ...
    // Live API calls for other endpoints
    try {
        ...
        // Return an object indicating an error instead of manipulating the DOM directly
...
const renderRestaurants = (restaurants) => {
    ...
    if (!grid) return;
    grid.innerHTML = restaurants.length === 0 ? '<p class="col-span-4 text-center text-gray-400">No restaurants found for this city.</p>' : '';
...
const renderDashboardView = async () => {
    ...
    if(initialRestaurants.error) {
        renderApiError('Popular Restaurants');
    } else {
        renderRestaurants(initialRestaurants);
    }
};
...
const renderSettingsView = () => {
    ...
    document.getElementById('settings-form').addEventListener('submit', (e) => {
        ...
        // In a real app, you would make an API call here to update user data.
        console.log('Saving user data (name, password)...');
