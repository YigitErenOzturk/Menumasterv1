...
const renderMenuView = async () => {
    ...
    return `<option value="${cat}" style="background-color: ${bgColor}; color: #9a3412; padding: 10px;">${cat}</option>`;
        ...
        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-orange-600">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" stroke-width="2" /></svg>
        </div>
    </div>
    
    <div id="new-category-container" class="hidden mt-2 flex gap-2 animate-fade-in">
        <input id="new-category-input" type="text" placeholder="Enter Category Name" 
        ...
        <button type="button" class="bg-red-50 text-red-500 font-bold px-3 rounded-lg hover:bg-red-100 transition" onclick="resetCategorySelection()">✕</button>
    </div>
</div>
          ...
          <div class="bg-white p-8 rounded-2xl border border-orange-100 shadow-xl">
            ...
            <form id="settings-form" class="space-y-5">
             ...
             <div>
                ...
                <div id="drop-area" class="relative border-4 border-dashed border-orange-100 rounded-2xl transition-all hover:border-orange-500 bg-gray-50 h-64 flex items-center justify-center overflow-hidden group cursor-pointer ${resData.imageUrl ? 'has-image' : ''}">
