import { supabase, throwSupabaseError, getSessionOrThrow } from '../../api/supabaseClient.js';
import { restaurantService } from '../../api/restaurantService.js';
    ...
    // Compatibility wrapper replacing the old localhost backend calls.
    const fetchData = async (endpoint, options = {}) => {
      try {
        const method = (options.method || 'GET').toUpperCase();
        const restaurantId = getRestaurantIdFromURL();

        if (endpoint === `restaurants/${restaurantId}` && method === 'GET') {
          const result = await restaurantService.getInfo(restaurantId);
          const restaurant = result.data;
          const menu = (await restaurantService.getMenuByRestaurant(restaurantId)).data;
          const reviews = (await restaurantService.getReviews(restaurantId)).data.reviews || [];
          return { ...restaurant, menu, reviews };
        }

        if (endpoint === `restaurants/menu/${restaurantId}` && method === 'GET') {
          return { menu: (await restaurantService.getMenuByRestaurant(restaurantId)).data };
        }

        if (endpoint === `reviews/restaurant/${restaurantId}` && method === 'GET') {
          return await restaurantService.getReviews(restaurantId);
        }
        ...
        if (endpoint === 'reservations' && method === 'POST') {
          ...
          throwSupabaseError(profileError, 'Please login as a user first.');
          ...
          throwSupabaseError(error, 'Could not create reservation.');
          return data || { ok: true };
        }
        ...
        if (endpoint === 'reviews' && method === 'POST') {
          ...
          const { data, error } = await supabase.from('reviews').insert({
            user_id: profile.id, restaurant_id: body.restaurantId, rating: body.rating, comment: body.comment
          }).select().single();
          throwSupabaseError(error, error?.code === '23505' ? 'You already reviewed this restaurant.' : 'Could not post review.');
          return data || { ok: true };
        }

        const reviewMatch = endpoint.match(/^reviews\/(\d+)$/);
        if (reviewMatch) {
          const id = Number(reviewMatch[1]);
          const session = await getSessionOrThrow();
          const { data: profile, error: profileError } = await supabase.from('profiles').select('id').eq('auth_user_id', session.user.id).single();
          throwSupabaseError(profileError, 'Please login first.');
          ...
          if (method === 'PUT') {
            const body = JSON.parse(options.body || '{}');
            const { data, error } = await supabase.from('reviews').update({ rating: body.rating, comment: body.comment })
              .eq('id', id).eq('user_id', profile.id).select().single();
        ...
        return { error: true, message: error.message || 'Operation failed.' };
      }
    };
    ...
    const renderPage = (data) => {
      ...
      // IMPORTANT: review.userId waiting
              ...
              <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-green-700 mt-1">
                    ⭐ ${myReview.rating} — “${escapeHtml(myReview.comment)}”
                  </p>
                </div>
          ...
          <div class="lg:col-span-1 space-y-8">
            <aside class="bg-white p-6 rounded-2xl shadow-lg border border-orange-100">
              ...
              <ul class="space-y-4 text-gray-600">
                <li class="flex items-start"><span class="font-medium">📍 ${escapeHtml(data.address || 'Address not listed')}</span></li>
  ...
  ${
    ...
    const handleReviewSubmit = async (e) => {
      ...
      if (!result.error) {
        ...
        // backend “already reviewed” it will show
